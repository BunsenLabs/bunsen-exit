...
class Config(object):
	...
	def read_style(self, style_path):
		style_entries = {}
		try:
			fh = open(style_path, "r")
			while True:
				line = fh.readline()
				if not line: break
				if 'style "window"' in line:
					style_class = 'window '
				elif 'style "button"' in line:
					style_class = 'button '
				elif 'style "label"' in line:
					style_class = 'label '
				elif 'style "tooltip"' in line:
					style_class = 'tooltip '
				if 'bg[ACTIVE]' in line:
					style_class += "ACTIVE"
					style_entries[ style_class ] = line[ -9:-2 ]
					exit_log.debug("style_entries " + style_class + " set to " + style_entries[ style_class ])
				elif 'bg[PRELIGHT]' in line:
					style_class += "PRELIGHT"
					style_entries[ style_class ] = line[ -9:-2 ]
					exit_log.debug("style_entries " + style_class + " set to " + style_entries[ style_class ])
				elif 'bg[SELECTED]' in line:
					style_class += "SELECTED"
					style_entries[ style_class ] = line[ -9:-2 ]
					exit_log.debug("style_entries " + style_class + " set to " + style_entries[ style_class ])
				elif 'bg[NORMAL]' in line:
					style_class += "NORMAL"
					style_entries[ style_class ] = line[ -9:-2 ]
					exit_log.debug("style_entries" + style_class + " set to " + style_entries[ style_class ])
				else:
			...
			exit_log.debug(err)
			exit_log.info("No style information found. Running with defaults.")
